...
function getModePad(modelJson, index) {
    ...
    return getApiModelPad(modelJson.title, modelJson.color || 'success', index, modelDom.innerHTML)
}

//获取一个Model外壳
function getApiModelPad(title, color, index, html) {
    let divDom = document.createElement('div')
    divDom.className = "panel panel-" + color
    divDom.style = ""
    divDom.innerHTML = `
                    ...
                    <div class="panel-body">
                        <div class="row">
                            <form action="${url}" method="${method}" class="col-sm-7">
                                ${inputs}
                                <br>
                                <button type="button" class="btn btn-success btn-sm" onclick="tryApi(this.parentNode)">测试接口</button>
                                <button type="button" class="btn btn-default btn-sm" onclick="tryApi(this.parentNode,true)">带空参数</button>
                            </form>
...
function tryApi(form, ignore) {
    ...
    for (let i = 0; i < temps.length; i++) {
        if (!!temps[i].name && (ignore || temps[i].value !== '')) inputs.push(temps[i])
    }
